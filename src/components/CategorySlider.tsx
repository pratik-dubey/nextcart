...
function CategorySlider() {
  ...
  useEffect(() => {
    const autoScroll = setInterval(() => {
      ...
      // scrollto is used to directly scroll to any point whereas scrollBy is used to scroll bit by bit
  ...
  return (
    ...
    >
      ...
      {showLeft && (
        <button
          className="absolute left-0 top-1/2 -translate-y-1/2 bg-white shadow-lg hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center transition-all"
          onClick={() => scroll("left")}
        >
